...
export async function buyFlashSale(
  ...
  if (idemExisting) {
    ...
    if (v?.state === "success") {
      ...
      return { status: 200 as const, body: v.body };
    }
    ...
    throw new HttpError(
      ...
      "Request dengan idempotency key yang sama masih diproses"
    );
  }
  ...
  if (userCount === 1) await redis.expire(userRlKey, 2);
  ...
  try {
    ...
    if (already) {
      throw new HttpError(
        403,
        "already_bought",
        "User sudah pernah membeli produk ini"
      );
    }
    ...
    const remaining = (await redis.eval(LUA_DECR_STOCK, 1, stockKey)) as number;
    if (remaining < 0) {
